...
function website_settings()
{
	...
	if($result)
	{
		...
		{
			if($row['setting_name'] == "Website Title") { $GLOBALS['website_title'] = $row['setting_value'];  }
			if($row['setting_name'] == "Website Subtitle") { $GLOBALS['website_subtitle'] = $row['setting_value'];  }
			if($row['setting_name'] == "Medium Thumb-Size") { $GLOBALS['medium_thumb_size'] = $row['setting_value'];  }
			if($row['setting_name'] == "Small Thumb-Size") { $GLOBALS['small_thumb_size'] = $row['setting_value'];  }
			if($row['setting_name'] == "Facebook Link") { $GLOBALS['facebook_link'] = $row['setting_value'];  }
			if($row['setting_name'] == "Twitter Link") { $GLOBALS['twitter_link'] = $row['setting_value'];  }
			if($row['setting_name'] == "Instagram Link") { $GLOBALS['instagram_link'] = $row['setting_value'];  }
			if($row['setting_name'] == "Tumblr Link") { $GLOBALS['tumblr_link'] = $row['setting_value'];  }
		}
	}
}
###########################################################################################################################
...
function site_settings_form()
{
	...
	while ($row = mysqli_fetch_assoc($result)) 
	{
		if($row['setting_name'] == "Website Title") { $website_title = $row['setting_value'];  }
		if($row['setting_name'] == "Website Subtitle") { $website_subtitle = $row['setting_value'];  }
		if($row['setting_name'] == "Medium Thumb-Size") { $medium_thumb_size = $row['setting_value'];  }
		if($row['setting_name'] == "Small Thumb-Size") { $small_thumb_size = $row['setting_value'];  }
		if($row['setting_name'] == "Facebook Link") { $facebook_link = $row['setting_value'];  }
		if($row['setting_name'] == "Twitter Link") { $twitter_link = $row['setting_value'];  }
		if($row['setting_name'] == "Instagram Link") { $instagram_link = $row['setting_value'];  }
		if($row['setting_name'] == "Tumblr Link") { $tumblr_link = $row['setting_value'];  }
	}
		
	echo '<form action="site_settings_submit.php?refresh=5" method="post">';
		echo '<fieldset>';
...
{
	$sql = 'SELECT * from posts where post_status = 1 AND post_video = 1 ORDER BY post_id DESC LIMIT '.$limit;
	$result = sql_select($sql);
	...
	if($result)
	{
		while ($row = mysqli_fetch_assoc($result)) 
		{
			echo '<div class="video">';
				...
				if(logged_in() == 'admin') { echo ' [<a href="edit_post.php?id='.$row['post_id'].'">Edit</a>]'; }
				echo '</div>';
...
function popular_tags($limit)
{
	$sql = 'SELECT tag_name, COUNT(*) as tag_count FROM tags GROUP BY tag_name ORDER BY tag_count DESC LIMIT '.$limit;
...
function recent_images($limit)
{
	global $small_thumb_size;
	$sql = 'SELECT * from images ORDER BY image_upload_date DESC limit '.$limit;
	$result = sql_select($sql);
	if($result)
	{
		while ($row = mysqli_fetch_assoc($result)) 
...
function edit_video_form($post_create)
{
	echo '<form action="edit_post_submit.php?refresh=10&url=index.php" method="post" enctype="multipart/form-data">';
		echo '<fieldset>';
			echo '<label>Title</label><br><input type="text" id="post_title" name="post_title" style="width:100%"></input><br>';	
			...
			echo '<input type="hidden" id="post_create" name="post_create" value="'.$post_create.'"></input>';
			echo '<label>Video Embed URL</label><br><input type="text" id="post_image" name="post_image" style="width:100%"></input><br>';	
			echo '<label>Content Summary</label><br><textarea id="post_summary" wrap="hard" name="post_summary" style="width:100%;height:300px"></textarea><br>';
			echo '<label>Tags</label><br><input type="text" id="post_tags" name="post_tags" style="width:100%"></input><br>';
			echo '<input type="submit" name="edit_button" value="Edit" />';
			echo '<input type="submit" name="delete_button" value="Delete" />';
		echo '</fieldset>';
	echo '</form>';
